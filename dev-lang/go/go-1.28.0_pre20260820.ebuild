...
# A prebuilt toolchain: stripping breaks Go binaries, and the port has
# no cgo, so nothing here links against C libraries.
RESTRICT="strip test"
QA_FLAGS_IGNORED=".*"
...
pkg_postinst() {
	...
	elog "It has no cgo: build with CGO_ENABLED=0. -buildmode is limited"
	elog "to exe, and the race detector is unavailable. go tool objdump"
	elog "and pprof's disassembly view do not work, as there is no"
